...
class Conv2dSamePad(nn.Module):
    ...
    def forward(self, x):
        ...
        pad_along_height = (
            (out_height - 1) * self.stride[0] + self.kernel_size[0] - in_height
        )
        pad_along_width = (
            (out_width - 1) * self.stride[1] + self.kernel_size[1] - in_width
        )
...
class ConvTranspose2dSamePad(nn.Module):
    ...
    def forward(self, x):
        ...
        return x[
            :, :, pad_top : in_height - pad_bottom, pad_left : in_width - pad_right
        ]
    ...
    def __init__(self, channels, kernels):
        ...
        for i in range(1, len(channels)):
            ...
            self.encoder.add_module(
                "conv%d" % i,
                nn.Conv2d(
                    channels[i - 1], channels[i], kernel_size=kernels[i - 1], stride=2
                ),
            )
        ...
        for i in range(len(channels) - 1):
            ...
            self.decoder.add_module(
                "deconv%d" % (i + 1),
                nn.ConvTranspose2d(
                    channels[i], channels[i + 1], kernel_size=kernels[i], stride=2
                ),
            )
...
class LinearAE(nn.Module):
    def __init__(self, channels, kernels):
        ...
        for i in range(len(channels) - 1):
            # Each layer will double the size of feature map
            self.decoder.add_module(
                "inv-linear%d" % (i + 1), nn.Linear(channels[i], channels[i + 1])
            )
            self.decoder.add_module("relud%d" % i, nn.ReLU(True))
...
class DSCNet(nn.Module):
    def __init__(self, channels, kernels, num_sample):
        ...
        self.n = num_sample
        self.ae = LinearAE(channels, kernels)
